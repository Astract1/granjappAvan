...
public class MapaCompradorActivity extends AppCompatActivity implements OnMapReadyCallback{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.mapaComprador);
        ...
        AutocompleteSupportFragment autocompleteFragment = (AutocompleteSupportFragment)
                getSupportFragmentManager().findFragmentById(R.id.autocomplete_fragmentComprador);
        ...
        autocompleteFragment.setOnPlaceSelectedListener(new PlaceSelectionListener() {
            ...
            public void onError(@NonNull Status status) {
                Toast.makeText(MapaCompradorActivity.this, "Error al seleccionar lugar", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        findViewById(R.id.btnUbiacionActualComprador).setOnClickListener(v -> {
        ...
        findViewById(R.id.btnBuscarPuntosCompradorComprador).setOnClickListener(v -> {
            // Obtener la dirección actual desde el EditText
            ...
            if (!direccionActual.isEmpty()) {
                // Inicializar un Geocoder para obtener las direcciones a partir de la dirección actual
                Geocoder geocoder = new Geocoder(this, Locale.getDefault());
                try {
                    List<Address> addresses = geocoder.getFromLocationName(direccionActual, 1); // Obtener una lista de direcciones basadas en la dirección actual
                    if (!addresses.isEmpty()) {
                        // Si se encontró al menos una dirección, obtener las coordenadas de la primera dirección de la lista
                        double latitud = addresses.get(0).getLatitude();
                        double longitud = addresses.get(0).getLongitude();

                        // Crear el intent y pasar la dirección y las coordenadas como extras
                        Intent intent = new Intent(this, CargarPuntosVentas.class);
                        intent.putExtra("direccion", direccionActual);
                        intent.putExtra("latitud", latitud);
                        intent.putExtra("longitud", longitud);
                        startActivity(intent);
                    } else {
                        Toast.makeText(this, "No se encontraron direcciones para la ubicación proporcionada", Toast.LENGTH_SHORT).show();
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                    Toast.makeText(this, "Error al obtener direcciones: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
            } else {
